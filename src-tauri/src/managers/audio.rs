...
// (the OS spinning up the audio device); keeping the stream warm for a few
// minutes means back-to-back dictations feel instant and only the first one
// after a long pause pays the open cost. 30s was too short for realistic "type
// a bit, dictate a bit" workflows.
